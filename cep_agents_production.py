...
# Try to import CopilotKit components
try:
    ...
    class LangGraphAgent:
        pass
    class CopilotKitConfig:
        pass
    print("Warning: CopilotKit LangGraph integration not available")
...
def create_performance_analyzer_agent():
    ...
    def analyze_node(state: CEPAgentState) -> CEPAgentState:
        ...
        if last_message and isinstance(last_message, HumanMessage):
            ...
            response = f"Performance analysis for {layer} over {period}:\n\n"
            response += f"Conversion Rate: {result['metrics']['conversion_rate']}\n"
            response += f"Response Time: {result['metrics']['avg_response_time']}\n"
            response += f"Error Rate: {result['metrics']['error_rate']}\n"
            response += f"Throughput: {result['metrics']['throughput']}\n\n"
    ...
    # Create CopilotKit config if available
    if COPILOTKIT_AVAILABLE:
        ...
        return SimpleLangGraphAgent(
            name="performance_analyzer",
            description="Analyzes CEP Machine performance metrics and provides optimization recommendations",
            graph=graph
        )

def create_business_growth_agent():
    ...
    workflow.add_edge(START, "prospect")
...
# Simple wrapper for when CopilotKit is not available
class SimpleLangGraphAgent:
    """Simple wrapper for LangGraph agents when CopilotKit integration is not available"""
    
    def __init__(self, name: str, description: str, graph):
        self.name = name
...
def get_agent(agent_name: str) -> Optional[Union[LangGraphAgent, SimpleLangGraphAgent]]:
...
if __name__ == "__main__":
    ...
    # Create test state
    test_state = CEPAgentState()
    test_state.messages = [HumanMessage(content="Search for prospects in San Francisco")]
    
    try:
        result = agent.invoke({"messages": test_state.messages})
